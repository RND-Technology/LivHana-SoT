...
import { getLightSpeedClient } from '../lib/lightspeed-client.js';
...
import { getCloudTasksClient } from '../lib/cloud-tasks.js';
...
function getLightspeedClient() {
  if (!lightspeedClient) {
    try {
      lightspeedClient = getLightSpeedClient();
...
function getCloudTasks() {
  if (!cloudTasks) {
    try {
      cloudTasks = getCloudTasksClient();
      logger.info('Cloud Tasks client initialized for post-purchase verification');
    } catch (error) {
...
router.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    const tasksClient = getCloudTasks();
    const taskName = await tasksClient.scheduleCountdownTimer(orderId, customerEmail, {
      orderId,
      customerId,
      orderTotal,
      deadline: deadlineISO
