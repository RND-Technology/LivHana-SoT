...
const ArrayCreate = require('./ArrayCreate');
const CreateDataPropertyOrThrow = require('./CreateDataPropertyOrThrow');
const GetMatchIndexPair = require('./GetMatchIndexPair');
...
const isStringOrUndefined = function isStringOrUndefined(s) {
	return typeof s === 'undefined' || typeof s === 'string';
};

const isMatchRecordOrUndefined = function isMatchRecordOrUndefined(m) {
...
module.exports = function MakeMatchIndicesIndexPairArray(S, indices, groupNames, hasGroups) {
	...
	const A = ArrayCreate(n); // step 5
	const groups = hasGroups ? OrdinaryObjectCreate(null) : void undefined; // step 6-7
	CreateDataPropertyOrThrow(A, 'groups', groups); // step 8

	for (let i = 0; i < n; i += 1) { // step 9
		const matchIndices = indices[i]; // step 9.a
		// eslint-disable-next-line no-negated-condition
		const matchIndexPair = typeof matchIndices !== 'undefined' ? GetMatchIndexPair(S, matchIndices) : void undefined; // step 9.b-9.c
