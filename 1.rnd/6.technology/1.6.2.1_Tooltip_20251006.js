...
const _DefaultTooltipContent = require("./DefaultTooltipContent");
const _TooltipBoundingBox = require("./TooltipBoundingBox");
...
const _selectTooltipEventType = require("../state/selectors/selectTooltipEventType");
...
function Tooltip(outsideProps) {
  const props = (0, _resolveDefaultProps.resolveDefaultProps)(outsideProps, defaultTooltipProps);
  const {
  ...
  const accessibilityLayer = (0, _accessibilityContext.useAccessibilityLayer)();
  const tooltipEventType = (0, _selectTooltipEventType.useTooltipEventType)(shared);
  const {
    activeIndex,
    isActive
  } = (0, _hooks.useAppSelector)(state => (0, _selectors.selectIsTooltipActive)(state, tooltipEventType, trigger, defaultIndexAsString));
  const payloadFromRedux = (0, _hooks.useAppSelector)(state => (0, _selectors.selectTooltipPayload)(state, tooltipEventType, trigger, defaultIndexAsString));
  const labelFromRedux = (0, _hooks.useAppSelector)(state => (0, _selectors.selectActiveLabel)(state, tooltipEventType, trigger, defaultIndexAsString));
  const coordinate = (0, _hooks.useAppSelector)(state => (0, _selectors.selectActiveCoordinate)(state, tooltipEventType, trigger, defaultIndexAsString));
  const payload = payloadFromRedux;
  const tooltipPortalFromContext = (0, _tooltipPortalContext.useTooltipPortal)();
  /*
  ...
  const [lastBoundingBox, updateBoundingBox] = (0, _useElementOffset.useElementOffset)([payload, finalIsActive]);
  ...
  const tooltipElement = /*#__PURE__*/React.createElement(_TooltipBoundingBox.TooltipBoundingBox, {
