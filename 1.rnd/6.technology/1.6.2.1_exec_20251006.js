...
function exec(nodemonOptions, execMap) {
  ...
  const scriptExt = path.extname(script).slice(1);

  let extension = options.ext;
  if (extension === undefined) {
    const isJS = scriptExt === 'js' || scriptExt === 'mjs' || scriptExt === 'cjs';
  ...
  if (
    ...
    const leadingArgs = (options.args || []).splice(0, options.scriptPosition);
