...
let child = null; // the actual child process we spawn
...
function run(options) {
  ...
  const runCmd = !options.runOnChangeOnly || config.lastStarted !== 0;
  if (runCmd) {
  ...
  const args = runCmd ? utils.stringify(executable, cmd.args) : ':';
  ...
  if (shouldFork) {
    ...
    const env = utils.merge(options.execOptions.env, process.env);
    stdio.push('ipc');
    const forkOptions = {
      env: env,
      stdio: stdio,
