...
Return the Number value that corresponds to value.
    */
	const sign = rawBytes[7] & 0x80 ? -1 : 1; // first bit
	let exponent = ((rawBytes[7] & 0x7F) << 4) // 7 bits from index 7
        | ((rawBytes[6] & 0xF0) >> 4); // 4 bits from index 6
	const mantissa = ((rawBytes[6] & 0x0F) * 0x1000000000000) // 4 bits from index 6
