...
function dictWithXmlEntries(itemSchema) {
  ...
  const modifiedSchema = __assign({}, dictSchema);
  modifiedSchema.unmapXml = function (inputValue, ctxt) {
    const output = dictSchema.unmapXml(inputValue, ctxt);
    ...
    const entries = objectEntries(output).map(function (_a) {
