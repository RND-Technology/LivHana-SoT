...
function _decorate(decorators, factory, superClass, mixins) {
  ...
  const r = factory(function initialize(O) {
  ...
  var api = {
    ...
    decorateElement: function (element, placements) {
      ...
      for (let decorators = element.decorators, i = decorators.length - 1; i >= 0; i--) {
        ...
        const elementFinisherExtras = this.toElementFinisherExtras((0, decorators[i])(elementObject) || elementObject);
    ...
    decorateConstructor: function (elements, decorators) {
      ...
      for (let i = decorators.length - 1; i >= 0; i--) {
        ...
        const elementsAndFinisher = this.toClassDescriptor((0, decorators[i])(obj) || obj);
    ...
    toElementDescriptor: function (elementObject) {
      ...
      const key = _toPropertyKey(elementObject.key);
    ...
    toElementFinisherExtras: function (elementObject) {
      const element = this.toElementDescriptor(elementObject);
      const finisher = _optionalCallableProperty(elementObject, "finisher");
    ...
    toClassDescriptor: function (obj) {
      ...
      const finisher = _optionalCallableProperty(obj, "finisher");
    ...
    runClassFinishers: function (constructor, finishers) {
      for (let i = 0; i < finishers.length; i++) {
        const newConstructor = (0, finishers[i])(constructor);
...
function _createElementDescriptor(def) {
  const key = _toPropertyKey(def.key);
  let descriptor;
  if (def.kind === "method") {
    descriptor = {
      value: def.value,
...
function _coalesceClassElements(elements) {
  const newElements = [];
  const isSameElement = function (other) {
