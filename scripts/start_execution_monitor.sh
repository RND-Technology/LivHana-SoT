...
tmux new-session -d -s "$SESSION" -n console "bash -lc 'echo [$(date -u +%FT%TZ)] ${AGENT} monitor started >> \"$LOG_FILE\"; tail -f \"$LOG_FILE\"'" >/dev/null 2>&1 || {
...
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
STATUS_DIR="$ROOT/tmp/agent_status"
LOG_DIR="$ROOT/logs/execmon"
SESSION_NAME="execmon"

mkdir -p "$STATUS_DIR" "$LOG_DIR"
LOG_FILE="$LOG_DIR/execmon_$(date +%Y%m%d_%H%M%S).log"
STATUS_FILE="$STATUS_DIR/exec.status.json"
...
d={"agent":"execution","phase":"${phase}","status":"${status}","started_at":time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),"finished_at":""}
...
if command -v tmux >/dev/null 2>&1; then
  if ! tmux has-session -t "$SESSION_NAME" 2>/dev/null; then
    tmux new-session -d -s "$SESSION_NAME" -n console "echo '[execmon] running'; tail -f '$LOG_FILE'"
  fi
fi

write_status "running" "ready"
echo "Execution monitor initialized (tmux=${SESSION_NAME})"
