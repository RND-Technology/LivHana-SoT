...
# Get GoDaddy API credentials from 1Password
get_api_credentials() {
    ...
    if [[ -z "$GODADDY_API_KEY" || -z "$GODADDY_API_SECRET" ]]; then
        log "${YELLOW}⚠️ GoDaddy API credentials not found in 1Password${NC}"
        log "${YELLOW}Please ensure 'GODADDY_API_KEY' and 'GODADDY_API_SECRET' items exist in 'LivHana-Ops-Keys' vault${NC}"
        error_exit "API credentials not available"
    fi
    
    if [[ -z "$GODADDY_API_KEY" || -z "$GODADDY_API_SECRET" ]]; then
        error_exit "GoDaddy API credentials are incomplete"
    fi
...
validate_api_access() {
    ...
    local response
    response=$(curl -s -w "%{http_code}" -o /tmp/godaddy_api_test.json \
...
update_domain_dns() {
    ...
    # Update DNS records
    local response
    response=$(curl -s -w "%{http_code}" -o /tmp/godaddy_update_response.json \
        -X PUT \
        -H "Authorization: sso-key $GODADDY_API_KEY:$GODADDY_API_SECRET" \
        -H "Content-Type: application/json" \
        -d "$dns_record" \
