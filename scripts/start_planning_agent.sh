...
# Anchor process for agent session (idempotent, low CPU)
...
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
STATUS_DIR="$ROOT/tmp/agent_status"
LOG_DIR="$ROOT/logs/planning"
SESSION_NAME="planning"

mkdir -p "$STATUS_DIR" "$LOG_DIR"
LOG_FILE="$LOG_DIR/planning_$(date +%Y%m%d_%H%M%S).log"
STATUS_FILE="$STATUS_DIR/planning.status.json"
...
data = {"agent":"planning","phase":"${phase}","status":"${status}","started_at": time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),"finished_at":""}
...
if command -v tmux >/dev/null 2>&1; then
  if ! tmux has-session -t "$SESSION_NAME" 2>/dev/null; then
    tmux new-session -d -s "$SESSION_NAME" -n console "echo '[planning] running'; tail -f '$LOG_FILE'"
  fi
fi

write_status "running" "ready"
echo "Planning agent initialized (tmux=${SESSION_NAME})"
